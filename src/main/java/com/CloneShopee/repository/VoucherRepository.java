...
public interface VoucherRepository extends JpaRepository<VoucherShop, Integer> {
        ...
        // @Query("SELECT p.product.id FROM VoucherShopItem p WHERE p.product.id IN
        // :productIds AND p.voucher.id = :voucherId")
        // Set<Integer> getProductIdsOfVoucher(@Param("productIds") Set<Integer>
        // productIds,
